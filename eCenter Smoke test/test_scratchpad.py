...
#eCenter_buttons.driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
...
eCenter_buttons.mydriver.find_element_by_xpath(xpaths['Lgn_UserField']).send_keys(username)

#Clear Password TextBox if already allowed "Remember Me"
eCenter_buttons.mydriver.find_element_by_xpath(xpaths['Lgn_PassField']).clear()

#Write Password in password TextBox
eCenter_buttons.mydriver.find_element_by_xpath(xpaths['Lgn_PassField']).send_keys(password)
...
eCenter_buttons.mydriver.find_element_by_xpath(xpaths['Menu_AdminFunc']).click()
